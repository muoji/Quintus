...
Quintus.UI = function(Q) {
  ...
  Q.UI.Text = Q.Sprite.extend("UI.Text", {
    ...
    draw: function(ctx) {
      ...
      for(var i =0;i<this.splitLabel.length;i++) {
        var x, y = -this.p.cy + i * this.p.size * 1.2;
        ...
        var txt = this.splitLabel[i];
        if(this.p.border) {
          ctx.fillStyle = this.p.border || "white";
          var w = this.p.borderWidth || 1;
          for(var i = 1; i <= w; i++) {
              ctx.fillText(txt,x-i,y);
              ctx.fillText(txt,x-i,y-i);
              ctx.fillText(txt,x,  y-i);
              ctx.fillText(txt,x+1,y-i);
              ctx.fillText(txt,x+i,y);
              ctx.fillText(txt,x+i,y+i);
              ctx.fillText(txt,x,  y+i);
              ctx.fillText(txt,x-i,y+i);
          ...
          ctx.fillStyle = this.p.color || "black";
            ...
            var dx = this.p.shadow.dx || 0;
            var dy = this.p.shadow.dy || 0;
            ctx.fillStyle = this.p.shadow.color || "black";
    ...
    font: function() {
      ...
      this.fontString = (this.p.weight || "800") + " " +
                        (this.p.size || 24) + "px " +
                        (this.p.family || Q.UI.defaultFont);
